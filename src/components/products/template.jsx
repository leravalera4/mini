...
>
<div
  style={{ //paddingRight: "20px", flexDirection: "row" 
    display: "flex",
    flexDirection: "column"
  }}
>
  {item.products.map((store, index) =>
    loading ? (
      ...
      >
        ...
        {/* {store.mem || store.saleprice !== null ? (
          <p
            className={noir.className}
            style={{ fontWeight: "700", color: "red" }}
          >
          ...
          <p
            ...
            style={{ fontWeight: "700" }}
          >
            {loading ? <Skeleton /> : store.regprice}
          </p>
        )} */}
        {loading ? (
          <Loading />
          ...
          >
            {store.mem}
            <span style={{marginLeft: "4px"}}>({store.saleprice})</span>
          </p>
        ) : store.saleprice != null ? (
          <p
            className={noir.className}
        ...
        {loading ? <Skeleton /> : <p>{store.stock}</p>}
        {loading ? <Skeleton /> : store.points}
        {loading ? <Skeleton /> : store.sale}
      </div>
    )
  )}
</div>
...
const Index = () => {
  ...
  const initialSteps = [
    {
      target: '.step-1',
      content: 'Это первый шаг',
      placement: 'right',
    },
    {
      target: '.step-2',
      content: 'Это второй шаг',
      placement: 'right',
    },
    {
      target: '.step-3',
      content: 'Это третий шаг',
      placement: 'right',
    },
  ];
  ...
  const handleJoyrideCallback = (data) => {
    ...
    if (status === 'finished' || status === 'skipped') {
      ...
      // Если текущий шаг первый, добавить второй шаг
      setSteps([initialSteps[0]]);
    } else if (index === 1) {
      // Если текущий шаг второй, добавить третий шаг
      setSteps([initialSteps[0], initialSteps[1]]);
    }
  };

  return (
    <div>
      <button onClick={startTour}>Начать тур</button>

      {/* Примерные элементы на странице */}
      <div className="step-1">Шаг 1: Нажмите, чтобы увидеть следующий шаг</div>
      <div className="step-2">Шаг 2: Следующий шаг появится после первого</div>
      <div className="step-3">Шаг 3: Этот шаг появится после второго</div>
